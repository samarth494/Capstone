...
const runCode = async (req, res) => {
  ...
  console.log(
    `[DEBUG] Received Run Request: Language=${language}, CodeLength=${code?.length}`,
  );
  ...
  if (!language || typeof language !== "string") {
    return res
      .status(400)
      .json({ success: false, output: "language is required." });
  }
  if (!code || typeof code !== "string") {
    return res
      .status(400)
      .json({ success: false, output: "code is required." });
  }
