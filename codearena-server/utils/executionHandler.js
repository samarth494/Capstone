...
const getProblem = async (problemId) => {
  try {
    ...
    console.error(
      `[ExecutionHandler] Failed to fetch problem from DB [${problemId}]:`,
      error.message,
    );
    return null;
  }
...
const executeSubmission = async ({
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    // Use problem's own validator if provided, otherwise plain lenient string match
    let isCorrect = false;
    if (typeof problem.validator === "function") {
      isCorrect = problem.validator(actual, expected);
    } else {
  ...
  console.log(
    `[ExecutionHandler][${roomId}] ${passedCount}/${testCases.length} passed in ${totalTime}ms`,
  );
