...
const addReplayEvent = (roomId, type, playerId, data) => {
    ...
    if (type === 'code_update') {
        ...
        if (lastEvent && lastEvent.type === 'code_update' && lastEvent.playerId.toString() === playerId.toString()) {
            const timeDiff = (Date.now() - battle.startTime) - lastEvent.timestamp;
            if (timeDiff < 2000) { // Update last event if within 2 seconds
...
const persistBattleResult = async (roomId, winnerSId = null) => {
    ...
    try {
        ...
        await updatePlayerStats(player1.user._id, winnerSId === player1.id, !winnerSId);
        await updatePlayerStats(player2.user._id, winnerSId === player2.id, !winnerSId);
...
const RANK_PRIORITY = { 'Bronze': 0, 'Silver': 1, 'Gold': 2, 'Platinum': 3, 'Diamond': 4 };
...
const socketHandler = (server) => {
    ...
    setInterval(() => {
        ...
        for (let i = 0; i < battleQueue.length; i++) {
            ...
            if (bestMatchIndex !== -1) {
                ...
                console.log(`Matched ${p1Data.user.username} (${p1Data.user.rank}) vs ${p2Data.user.username} (${p2Data.user.rank}) | Gap: ${minGap}`);
            }
        }
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join_queue', (userData) => {
            ...
            console.log(`User ${userData.username} [${userData.rank || 'Bronze'}] joined queue`);
            battleQueue.push({
                id: socket.id,
                user: userData,
                joinedAt: Date.now()
        ...
        socket.on('join_room', (roomId, userData) => {
            ...
            if (activeBattles[roomId]) {
                ...
                if (userData && userData.username) {
                    ...
                    console.log(`User [${userData.username}] joined room [${roomId}]. Joined: ${battle.joinedPlayers.size}/2`);
                } else {
                    console.log(`Socket [${socket.id}] joined room [${roomId}] without username.`);
                }
                ...
                if (battle.joinedPlayers.size === 2 && !battle.intervalId) {
                    console.log(`Both players joined ${roomId}. Starting timer for real now.`);
                    ...
                    battle.intervalId = setInterval(() => {
                        if (battle.timer > 0) {
                            ...
                            io.to(roomId).emit('battle:timerUpdate', { timeLeft: battle.timer });
                        } else {
                ...
                // Identify Opponent
                if (userData && userData.username) {
                    const opponent = battle.players.find(p => p.user.username !== userData.username);
                    if (opponent) {
                        socket.emit('battle:opponentInfo', { username: opponent.user.username });
                    }
                }
            } else {
                console.warn(`Client tried to join non-existent room: ${roomId}`);
                socket.emit('battle:error', { message: 'Battle session not found. The server may have restarted.' });
            }
        });
        ...
        socket.on('battle:submit', async ({ roomId, code, dryRun = false }) => {
            console.log(`Handling battle:submit [DryRun: ${dryRun}] for room ${roomId} from ${socket.id}`);
            ...
            const result = await executeSubmission({ roomId, userId: socket.id, code });
            ...
            const lastEvent = battle.replayEvents[battle.replayEvents.length - 1];
            if (lastEvent && lastEvent.type === 'submission' && lastEvent.data.code === code) {
